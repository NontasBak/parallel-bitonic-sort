...
void merge_up(Process* processes, int start_id, int size, int num_q) {
    ...
    for (int i = 0; i < cut; i++) {
        compare_up_elementwise(processes[start_id + i],
                               processes[start_id + cut + i], num_q);
...
void merge_down(Process* processes, int start_id, int size, int num_q) {
    ...
    // Compare element wise the blocks
    for (int i = 0; i < cut; i++) {
        compare_down_elementwise(processes[start_id + i],
                                 processes[start_id + cut + i], num_q);
...
// turns. For example size = 2 means that 2 blocks will be sorted increasingly, 2
// blocks will be sorted decreasingly etc.
